...
**Конвейер** (агенты пришпилены к opus/high — конфигурация сборки, SKILL.md хард-рул 3):
```
pool_dossier.py <пул> --pair <PAIR>                # (скрипт) линии + карточки карт
Workflow workflow_build.js {set,draft8,pair,pool,dossier,medians}
    → Линии: агент-проектировщик решает fan/single и пишет брифы (must/avoid/маршрут)
    → Сборка: по строителю на линию (без инструментов, только данные промпта)
скорборды: build_audit --pool · draft_goldfish · pool_dossier --deck   # (скрипты)
судья: отдельный агент — выбор по скорбордам + графты + 2-4 спорных слота
build_registry.py add <set> <draft8> <тег> <файл>  # кандидаты, судья, user, final
```
Выход пользователю: победитель + **2–4 реально спорных слота с ценой альтернативы** —
сравнение с его сборкой фокусируется на развилках, а не на перепроверке всех 23 карт.
После согласования финала: `build_registry.py add … final` — расстояние «первый кандидат →
финал» и есть метрика процесса (эталон 20.08: advisor→final = 4 и 1; trio § 8.19 = 7).
