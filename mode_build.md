...
| 3 | **Существо к T2 — РАЗМЕН с осью «тел силой ≥4» (§ РАЗМЕН), сверять с популяцией, а не с 60%** (медиана победителей **54.3%**, мин 36.4%; жёсткий ≥60% СНЯТ 10.08 — `rules_graveyard` §1–2) | ref_decks/ n=23; наши лучшие 70–77% = топ-3 популяции; вылет 11.07 — 40% |
...
**🚨 СТОП-КРАНЫ (перекрывают счёт):** провален порог **6 (закрывашка)** → **потолок 7, как бы ни было остальное** («загнал, но не добил» — документирован чисто). Порог **3** — стоп-кран ТОЛЬКО для полосы **C** (агро без 5–6 дешёвых тел не работает) и при падении **ниже минимума популяции (<40%)**; для полос A/B это ориентир: у победителей медиана 54.3%, жёсткие ≥60% сняты 10.08 (`rules_graveyard` §2) — **ссылаться на них как на кран нельзя** (ошибка повторена 20.08, см. § НЕ РЕЗАТЬ РОЛЬ РАДИ МЕТРИКИ). Failure-mode «переехали до того, как развернулся» в матч-логах реален, но почти всегда с ко-категорией MANA/VARIANCE — это про руки и ману, а не про состав 23.
