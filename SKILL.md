...
- **Castability beats raw GIH.** `✗offcolor` = карта не существует, пока нет фикса. `~splash` = реально только при 3+ источниках (Rule of Three). Дабл-пип off-color (напр. {B}{B}) не сплешится **вообще**. Исключение: `✗offcolor` на пике ≤5 при 0 коммите = сигнал ПИВОТИТЬ, а не брать.
