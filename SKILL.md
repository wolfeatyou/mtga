...
**Цель и философия (ОБНОВЛЕНО 08.07.2026 — новый подход с Platinum):**
С Platinum+ **НЕ строим «boring» надёжный 2-цветный removal-pile. Строим TOP-деку с ПОТОЛКОМ:** бомбы/хеймейкеры + ответы + кривая. Floor (removal/кривая) — фундамент, но на высоких рангах у оппонентов бомбы, и floor-only проигрывает их ceiling. Reconcile с «be boring»: пол остаётся базой, но НЕ ценой потолка. Никогда не собирай bomb-less синергия/removal-пайл (это давало 0:3 стрики).
1. **CEILING > FLOOR.** Вопрос к каждому пику: **«это ВЫИГРЫВАЕТ игры или просто solid?»** Бомба/жёсткий эвейжн-клок выше N-го removal/филлера. Пол не заменяет потолок.
2. **ОСТАВАЙСЯ ОТКРЫТЫМ до P2/пик 3 (late-commit).** На P1P1–P1P5 сигнала об открытости ещё НЕТ (свежие паки, соседи слева не пикали) → бери силу/гибкость/colorless/фикс, НЕ лочь 2 цвета. С пика 6+ читай что течёт/колесит → определяй открытый цвет. Коммит на P2P3. Будь готов **пивотнуть В ЦВЕТ БОМБЫ** (иначе «нас убивают картами, которых мы не взяли»). Голд P1P1 = самый закрытый пик (лочит 2 цвета) — бери только если хеймейкер заметно выше лучшей гибкой карты; иначе моно/colorless. Иерархия гибкости: colorless > моно > голд.
3. **Синергия — ТОЛЬКО та, что ЗАКРЫВАЕТ** (пейофф-финишёр, не value-движок). Value-синергия без закрывашки = durdle-trap. Пейофф требует ПЛОТНОСТИ (Villain/Hero-трайбал, engine) — считай density ПЕРЕД пиком; нет плотности → пас.
4. **Identify the anchor in P1–P3.** Якорь — высокоIWD бомба/движок. Каждый пик: «кормит ли это якорь / поднимает потолок?»
