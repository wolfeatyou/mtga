...
```
**Сохранять ВЕСЬ пул (мейн + сайд) каждой нашей колоды** — без остатка пула решение о срезе не проверяется ни у нас, ни у них.

**2. Тест процесса: наш мейн не должен совпадать с «жадным» топ-23 по GIH.** Ни одна из 14 победивших колод не равна жадному списку — все отдают **+0.53 GIH на карту, 14 из 14 в одну сторону** (случайно — 1 шанс из 16 000). Совпадение с жадным = «план не выбран, пул отсортирован». Печатает `build_audit.py`. **Этот тест про НАШ процесс, поэтому он не страдает от survivorship-bias референс-выборки** — в отличие от любых выводов вида «4 цвета выигрывают», которые из выборки только победителей делать НЕЛЬЗЯ.
...
| 4 | ~~Hard removal ≥4~~ → **ПОНИЖЕН 10.08.2026 до «разбери по категориям и назови число»**. Порог ≥4 опровергнут: у 14 листов 7-1/7-2 медиана безусловного removal **1**, у пяти колод **ноль**. Разбивка по категориям (см. ниже) остаётся полезной, само число «≥4» — нет | было: трофейки 7, PT 6 (n=5, только наши) |
...
4. **`build_audit.py <лист.txt>` — ГЛАВНАЯ проверка (§ КАЛИБРОВКА).** Лист обязан содержать `Sideboard` с остатком пула. Даёт (а) положение по каждой оси в распределении 14 победителей — чинить только то, что ниже ВСЕЙ популяции; (б) тест «мейн ≠ жадный топ-23». **Совпадение с жадным = плана нет, вернуться к § Креативный проход.**
