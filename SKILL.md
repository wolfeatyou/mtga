...
> **Проблема:** тул печатает пак **отсортированным по GIH**, и рассуждение всегда начиналось с верхней строки, а дальше было её рационализацией. Задокументировано дважды за один драфт: P1P2 (взял бы карту с равным GIH, но втрое худшим IWD) и P3P1 (`ЛОГИКА` дословно «топ по GIH среди истинно доступных карт» при `пик C-` в той же строке). Правило «GIH — старт, не ответ» стояло в файле и не работало: **сортировка сильнее прозы.**
...
>
> **🔴 НО ПОРОГА ИЗ ЭТОГО НЕ ВЫШЛО — гипотеза «≥5 бесплатных повторяемых» ОПРОВЕРГНУТА в день внесения.** Замер по 23 листам `ref_decks/` (скрипт-эвристика: триггер `Whenever/At the beginning` без стоимости активации): **мин 2 · медиана 4 · макс 9, и 16 из 23 победителей НИЖЕ пяти.** При этом наша колода 0W–3L имела **ровно 5** — то есть выше медианы победителей. Порог отбраковал бы 2/3 выигравших колод и пропустил нашу. Классический рецидив «обучаемся на своих сливах» (§ КАЛИБРОВКА п.1), от которого этот файл и защищается.
> Дополнительно: **power-up НЕ является разделителем** — победители держат их столько же (медиана ≈4, у нас было 5).
>
> **Что остаётся в силе:** это разумный **тайбрейк при примерно равном GIH**, а не измеряемый порог. Бесплатный повторяемый триггер лучше однократного мана-синка при прочих равных — но статистически он НЕ отличает выигравшие колоды от наших.
>
...
| # | Порог | Откуда |
|---|---|---|
| 1 | **Двуролевых ≥15 из 23** | PT-эталон ~24/24; наши провалы ~треть. *(Подпорог «≥5 бесплатных повторяемых» вводился 11.08.2026 и **снят в тот же день** — опровергнут на `ref_decks/`, см. ниже)* |
