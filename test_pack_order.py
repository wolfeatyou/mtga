...
ЧТО ПРОВЕРЯЕТСЯ:
  A. при равном GIH выше встаёт карта с большим IWD (док. случай Take Up the Shield /
     Super-Skrull, MSH 10.08.2026, P1P2);
  B. некастуемые уходят в отдельную группу ВНИЗ, а не стоят первыми по GIH;
  C. GIH остаётся ведущей осью — карта, лучшая по обеим, первая (вес 2:1, не 1:1);
...
show("A. Равный GIH (59.9 обе), IWD +3.0 против +8.6 — MSH P1P2, 10.08.2026", g)
...
for hi, lo in pairs:
    ...
    check(flat(g)[0] == hi, "заметно лучший GIH побеждает — вес 2:1, IWD не переворачивает всё")
...
print("   · равный GIH разрешается по IWD (промах P1P2 10.08.2026 больше невозможен)")
print("   · некастуемые в своей группе внизу, а не наверху по GIH")
print("   · GIH остаётся ведущей осью — IWD только тайбрейк")
