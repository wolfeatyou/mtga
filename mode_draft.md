...
> **🔎 ТЕКСТ КАРТЫ — ЕДИНСТВЕННЫЙ ИСТОЧНИК МЕХАНИКИ (внесено 20.08.2026).** Под каждой
> картой пака печатается ПОЛНЫЙ оракл-текст (обрезка 280 снята — она резала 32 карты HOB,
> включая removal-текст Azog и главы саг). Сет может быть новее кат-оффа модели: рассуждай
> от напечатанного текста, никогда от памяти; карта помечена ⚠ НЕ РАСПОЗНАНО — спроси
> текст у игрока, а не восстанавливай по названию (§ 8.16: «value-движок приписан
> защитной саге без чтения текста»).
...
```bash
cat ~/.claude/skills/mtg-draft-helper/<set>_learned.md     # ЧТО РЕАЛЬНО ИГРАЮТ: ядро каждой пары, роли, кривая (автогенерация из ref_decks/)
...
The snapshot prints cards **grouped by castability, then by GIH + set colour adjustment + bomb bonus** (NOT a power ranking — see § «первая строка не аргумент»), each with `[tier|GIH xx.x|<PAIR> xx.x|IWD ±y.y|OH zz.z|ALSA a.a]` plus pool-aware flags, the booster/pick number, and the user's full **POOL** (colors + curve).
...
- **🆕 `пик <тир>` — КАК КАРТУ РЕАЛЬНО БЕРУТ в Diamond→Mythic** (untapped.gg pick order, `msh_pick_tiers.json`, 196 карт, снято 10.08.2026). **Это ортогональная GIH ось: не «сколько выигрывает», а «сколько за неё дают».** Расхождение между `GIH` и `пик` — и есть содержательная часть пика.
  - **Измерено (n=196):** ρ(тир пика, GIH) = **+0.79** — GIH остаётся лучшим одиночным предиктором, спорить с ним не надо.
  - **🪦 IWD КАК ТАЙБРЕЙК ПОХОРОНЕН 18.08.2026** (`rules_graveyard.md` § 9, JOURNAL § 8.9). Здесь стояло правило «при примерно равном GIH решает IWD», выведенное из ρ(тир, IWD) = +0.42…+0.66 внутри полос GIH. Оно проверено на 294k спорных паков MSH и **опровергнуто**: на GIH-ничьих (|Δ|≤0.5) с различающим IWD (|Δ|≥1.5) трофейщики берут высокий IWD в **52.2%** (монетка), а провальщики — в **55.4%**, то есть ЧАЩЕ. Ось `2·GIH+1·IWD` попадает в пик трофейщика не лучше чистого GIH (31.6% против 31.8%). Из кода IWD снят и из сортировки, и из причин ⚑ТАЙБРЕЙК.
  - **Чем решается ничья на самом деле — ЦВЕТОМ ПУЛА** (+8 п.п. попадания в пик трофейщика, § 8.9 этап 4). Это уже делает группировка пака по кастуемости, и она данными подтверждена. Тайбрейк ⚑ баннера — **только пик-тир**; на HOB `hob_pick_tiers.json` не снят, поэтому баннер там молчит и ничью решает роль/дыра/квадранты, а не второй знак числа.
  - ⚠️ **Не воскрешать**: правило было вычищено отсюда 20.08.2026 после того, как обнаружилось, что оно пережило собственные похороны на два дня и решило пик P1P4 живого драфта eba1b036 (Confusticate 57.0/IWD +7.9 над Smaug 57.2/+1.3, обоснование дословно «IWD — один из самых высоких выбросов пака»).
...
- **🔴 ЦВЕТА ПУЛА И ГИБРИДЫ: гибридный пип даёт ДОСТУП, а не принадлежность (починено 20.08.2026).** Флаги, `⚑ ОСЬ`, «⚠ НЕ В ЭТОЙ ПАРЕ» и `⚑ ПИВОТ` — все считаются от `main` (топ-2 цвета пула), и до починки `{2}{W/U}` голосовала за белый наравне с настоящей белой картой. **Док. случай (драфт eba1b036):** четыре `Patient Instructor` дали W:7 против R:6, и инструмент **весь второй и третий бустер** называл UR-колоду «WU» — печатал чужую ось, сверял «не в этой паре» не с той парой, а собственные красные карты (`Gandalf, Spark Starter`, `Pinecone Strike`, `Glóin`) уводил в `~splash`/`✗offcolor` и предлагал пивот из цвета, в котором мы и были. Теперь считается в два прохода: жёсткие пипы задают лидеров, гибрид отдаёт голос тому своему цвету, который уже в лидерах. **Читать так же и глазами:** гибрид в пуле — не заявка на цвет; его текст оценивается по плотности МОЕГО пула (см. `hob_insights.md`, `Thranduil, Sindarin Liege` в WU).
- **`─── СИГНАЛЫ ───` banner (printed ABOVE the pack by the snapshot — read and relay it first).** The snapshot auto-detects what the per-pick GIH sort can't, the exact class of mistakes that cost real drafts:
  - **`⚑ ПОСЛЕДНИЙ ПИК` — ПЕРВАЯ строка блока, печатается каждый пик (внесено 20.08.2026).** Называет карту, реально ушедшую в пул на прошлом ходу, и её номер. **Сверь со своим прошлым советом ДО того, как думать над новым паком**; разошлось — скажи вслух и учти в плане. Повод: в драфте eba1b036 на P2P8 советчик рекомендовал `Gollum the Abandoned`, игрок взял `Gandalf, Wandering Wizard` (50.5, ALSA 8.1) — и расхождение не заметили до конца драфта, хотя имя стояло в хвосте печатаемого пула. Двадцать следующих советов строились на пуле, который никто не сверял. **Номер пика важен:** совпал с номером текущего пака — значит игрок успел пикнуть до рендера, и сверять надо с советом на пак раньше.
  - **`⚑ КОПИИ` — сколько копий этой карты вообще играют победители (внесено 20.08.2026).** Загорается, когда в пуле уже столько копий карты из пака, сколько максимум встречено у трофейщиков (сначала — максимум ВНУТРИ пары, при отсутствии данных — по сету). **Это не запрет:** у победителей бывает и 8 копий одной карты (`Lakeshore Apothecary`). Баннер говорит ровно одно — «дальше ты выходишь за пределы наблюдавшегося», решение остаётся за планом. Повод: в eba1b036 набрано 4 `Confusticate and Bebother` при максимуме 3, 3 `Old Thrush` и 3 `Long-Bodied Grey Dog` при максимуме 2, и **ни один прибор не сказал об этом в момент пика**. Источник — блок `played` в `<set>_traps.json` (`find_traps.py <set> --write`).
  ...
  - **⚖️ `⚑ ТАЙБРЕЙК` — срабатывает, когда GIH-сортировка ставит наверх НЕ ту карту (внесено 11.08.2026).** Молчит, пока верх по GIH и верх по тайбрейку — одна карта. Говорит, когда в пределах **1.5 GIH** есть карта заметно лучше по **пик-тиру** (≥2 ступени) — и называет её поимённо. Некастуемые в тайбрейке не участвуют. **IWD из причин баннера СНЯТ 18.08.2026** (см. выше и `rules_graveyard.md` § 9) — осталась одна причина, пик-тир. **На HOB пик-тиров нет, поэтому баннер молчит весь сет** — это не поломка, а отсутствие данных.
    - **Повод — задокументированный промах (Quick MSH, 10.08.2026, P1P2):** в паке лежали `Take Up the Shield` GIH 59.9 / пик C+ и `Super-Skrull` GIH **59.9** / пик **B**, 4/5 flying. **GIH совпал до десятой**, советчик взял верхнюю строку списка. Игрок взял Super-Skrull вопреки совету и был прав: колода уехала в воздух (флай 8). Этот же случай новая ось решает в ту же сторону **через цветовую поправку**, а не через IWD.
  ...
  - **`⚑ КРИВАЯ` — печатается КАЖДЫЙ пик — ИНФОРМАЦИЯ, НЕ ПРИКАЗ.** Показывает `существ cmc≤2 — N · чекпойнт K · финал ≥5`, считая только тела, **кастуемые в текущих цветах**. Читать как **дескриптор полосы**: агро-полоса C без 5–6 дешёвых тел не работает, бомбовая/соуп-колода спокойно живёт на 40–50%. Число НЕ является порогом качества колоды (у победителей медиана 54.3%) и пик им не решается — история понижения в `rules_graveyard.md` §1.
    - **🔴 При `— НЕДОБОР` баннер называет дешёвые тела в паке И ПОМЕЧАЕТ, ИГРАЮТ ЛИ ИХ ПОБЕДИТЕЛИ ПАРЫ (починено 20.08.2026).** `✔карта (UR 54%)` — тело из ядра пары, оно роль закрывает. `✗карта (UR 0%)` — тело, которого победители пары не играют: **им квота НЕ закрывается**, бери карту ядра даже при более низком GIH. Частоты берутся из `<set>_traps.json` (блок `played`, пишет `find_traps.py <set> --write` по `ref_decks/`); сет без этого файла помечается `✔` по умолчанию — «нет данных» не повод обвинять карту.
    - **Повод — драфт eba1b036 (20.08.2026):** баннер печатал «ПРАВИЛО: берём дешёвое тело, обходится ТОЛЬКО бомбой ≥63 или безусловным removal» и трижды за драфт указал на `Old Thrush` (GIH 50.9, **0 из 14 UR-листов победителей**, 23/298 по сету). Все три раза его взяли — **19.3 GIH-пункта, треть всей упущенной ценности драфта**. Поправка была записана в `<set>_insights.md` ещё 17.08, но осталась прозой, пока приказ стоял в баннере. Слово «ПРАВИЛО» из баннера снято: порог понижен до ориентира ещё 10.08 (`rules_graveyard.md` §1), а звучал он как стоп-кран.
...
ПИВОТ: {решение + ЦЕНА числом}                  ← ⚑ОБЯЗАТЕЛЬНА, пока горит ⚑ПАСУЕМ/⚑ПИВОТ (см. ниже)
...
- **🚫 «Выше по GIH» / «первая в списке» — ЗАПРЕЩЁННОЕ обоснование в `ЛОГИКА`.** **Обосновывай ролью, планом полосы, квадрантами или дырой.** Берёшь карту с низким пик-тиром — скажи, почему ЗДЕСЬ она лучше, чем её берут в среднем. Сработал `⚑ ТАЙБРЕЙК` — адресуй его явно, как ⚑-баннеры.
  - **⚠️ И «первая строка» теперь тем более не аргумент: с 16.08.2026 порядок печати — НЕ рейтинг силы.** Пак идёт группами по кастуемости, внутри — по `GIH + цветовая поправка сета + бомба-надбавка` (`draft_live.pack_order`; IWD из оси снят 18.08.2026, **на HOB цветовой поправки нет — там ось это чистый GIH + бомба**). Верхняя строка означает «лучшая по числу среди кастуемых», а не «пик». Источник якоря убран в коде, потому что прозой не лечился (`rules_graveyard.md` §8).
...
- **🔴 НО пока горит `⚑ ПАСУЕМ <цвет>` или `⚑ ПИВОТ?` — строка `ПИВОТ:` ОБЯЗАТЕЛЬНА И НЕ СОКРАЩАЕТСЯ
  (внесено 16.08.2026).** Это единственное поле блока со статусом «обязательное при условии»;
  всё остальное по-прежнему печатается только когда есть что сказать.
  - **Формат — решение + цена ЧИСЛОМ, а не отговорка:**
    `ПИВОТ: сплешу B (Desolation Prowler 61.1, 1 пип) — фикс 2/3, добираю третий` ·
    `ПИВОТ: отказ — цена пивота 11 карт, до конца 8 пиков` · `ПИВОТ: ухожу в B, теряю 4 карты`.
  - **Запрещённые ответы:** «поздно», «мы уже глубоко», «остаёмся в цвете» без числа.
    § ЦВЕТ ВЫБИРАЮТ СИГНАЛЫ прямо требует проверять глубину **числом**, а не ощущением:
    colorless и вторые копии при пивоте не теряются.
  - **Почему это вынесено в жёсткое правило (причина техническая, а не стилистическая).**
    В драфте 31a78cee поле `ПИВОТ` было помечено как опциональное — и при экономии вывода
    опциональное выпадает первым. Вместе со строкой пропала и обработка сигнала: все 13
    баннеров остались без ответа. Дело не в том, что советчик «мало думал» — ему **нечего
    было заполнять**, и шаг тихо исчез. Обязательное поле стоит одну строку и заставляет
    сигнал обработать; на Sonnet/medium, где бюджет внутреннего рассуждения меньше, чем на
    Opus/high, структура вывода — единственный надёжный чек-лист.
...
> **⚠️ GU: «доп. земельные дропы» — снятая дефицитная ось (17.08.2026).** Замер по 31 трофейной
> GU-колоде: landfall-карт **5.32 на колоду** — вторая плотность во всём сете после BR-amass
> (6.54). Карт с доп. дропом всего две (`Thranduil's Company`, `Beorn, Reluctant Host`), и
> **15 из 31 колод-победителей не держат ни одной** — при этом landfall у них даже плотнее
> (**5.47** против 5.19 у держащих). То есть механику кормит обычный дроп земли в ход, а не
> редкая карта, и приоритет пика на неё вешать не за что. Освободившийся приоритет — под
> пейофф-тела (`Thranduil, Sindarin Liege`, `Silvan Reveler`, `Attercop`, `Boughside Wanderers`).
> `Thranduil's Company` остаётся сильным пиком сам по себе — но не по причине «кормит landfall».
   ...
   - **🔴 На P3P1 аудит МЕНЯЕТ ВОПРОС: назови СИЛЬНЕЙШУЮ ЛИНИЮ ПУЛА — и посвяти ей весь третий бустер (директива игрока 22.08.2026, драфт 91a4b8e8).** Не «какую пару я веду» и не «какие дыры латать», а **«какая линия в этом пуле объективно сильнейшая»** — по осям (⚑ОСЬ / ⚑ОСЬ-КОНФЛИКТ), маршрутам (⚑ПЛАН ПОБЕДЫ) и качеству карт. Ведомая ≠ сильнейшая → менять (второй цвет — дёшево, см. лестницу § ПРАВИЛО НЕ ЗАКАНЧИВАЕТСЯ). Б3 тратится на УГЛУБЛЕНИЕ названной линии: деталь её оси > дженерик чуть выше по GIH (бюджет — § эксперимент «приоритет оси»); добор «ровно по чуть-чуть всего» на Б3 — это и есть сборка мягкой середины. Док. случай: в 91a4b8e8 сильнейшей линией к Б3 были Волки/4-сила, советчик вёл UB и тратил пики на вторые копии дженерик-пейоффов — линию назвал ИГРОК, её колода взяла 5W-3L.
   - **(b) Креативный пересмотр — ищи ЛУЧШУЮ колоду в пуле, а не только латай дыры текущего плана.** Спроси себя:
     · **🔴 ПЕРВЫМ: топ-ось пула (⚑ ОСЬ) → её ПАРА → совпадает с ведомой? (внесено 22.08.2026, драфт 91a4b8e8.)** Инерция коммита: ярлык пары из Б1 не пересматривается, хотя оси пула уже голосуют за другую. В том драфте «Волк 5-7» печатался с середины Б2 (ось BG), советчик вёл UB до P3P5 — пивот сделал ИГРОК, и BG-ядро взяло 5W-3L. **При общем базовом цвете цена смены = только ВТОРОЙ цвет, а гибриды не теряются вовсе** — это почти всегда дешевле, чем кажется. Баннер `⚑ ОСЬ-КОНФЛИКТ` (с пула ≥12, ось ≥4 и плотнее своей) теперь **считает вердикт САМ**: конфиг чужой пары держит ≥20 плейблов → печатает «пивот ОБЯЗАТЕЛЕН» с числами. **Вердикту следовать БУКВАЛЬНО: потери при смене баннер считает и печатает САМ; «отказ НЕлегален» = пивот выполняется; отказ возможен только когда баннер сам напечатал бомбу (GIH≥60) в потерях.** Судить потери своими словами запрещено — в прогоне v2 (§ 8.35) агент назвал бомбами карты 57-58 GIH и «легально» отказался. Почему числом, а не прозой: в переигровке 91a4b8e8 (§ 8.34-8.35 JOURNAL) агент отвечал на прозаический баннер формально и отказывался с ложной арифметикой («цена ~8 карт» — не посчитано, ядро общее; «фикса нет» — смена ПАРЫ фикса не требует, второй цвет кастуется с базовых земель).
...
> **Проблема:** рассуждение начиналось с верхней строки пака, а дальше было её рационализацией. Правило «GIH — старт, не ответ» стояло в файле и не работало: **сортировка сильнее прозы.** **Половина причины устранена 16.08.2026 в коде** (`pack_order`: группы по кастуемости + GIH с поправками, порядок помечен как не-рейтинг). Вторая половина — этот порядок рассуждения, и он остаётся в силе: любой отсортированный столбец якорит, каким бы ни был ключ сортировки.
...
5. **И только теперь — сверка с GIH / `пик <тир>` / ⚑ТАЙБРЕЙК.** Совпало — молчим. **Разошлось сильно — назвать почему** (в `ЛОГИКА`). Это проверка на грубую ошибку, а не источник решения. **IWD в сверку НЕ входит** — он печатается как информация и работает только через флаг `⚠trap` (IWD<0); тайбрейком быть перестал 18.08.2026.

> ### 🔴 ГОРИТ `⚑ ОЧЕВИДНЫЙ ПИК` — ШАГИ 1–4 НЕ ПЕРЕОПРЕДЕЛЯЮТ ЧИСЛО МОЛЧА (внесено 20.08.2026, драфт 3657e8ab, подтверждено игроком)
> Измерено на 2.18 млн пиков (§ 8.9 JOURNAL): провальщики теряют НЕ на спорных паках, а на
> **очевидных** — при отрыве топа ≥3 GIH они берут его в 56.7% против 64.9% у трофейщиков.
> Баннер печатает такой отрыв до совета (той же осью, что сортирует пак). Пока он горит:
> - **Топ первой группы — ответ по умолчанию.** Отклонение легально только с причиной,
>   названной в `ЛОГИКА`, из закрытого списка: дыра-роль · бюджет оси (с P2P1, ≤4/≤6) ·
>   квадранты · сработавший ⚑-баннер (ЛОВУШКА/КОПИИ/КРИВАЯ-ядро).
> - **«Не мой цвет» при пуле младше `pool_main_colors` (<5 пиков) причиной НЕ является.**
>   Пул из трёх карт — не обязательство; § 8.10 измерил, что все (и трофейщики тоже) теряют
>   ~5 из 14 пиков Б1, а тайминг коммита исход не двигает. Держать «цвета» ценой топ-1 —
>   это платить за обязательство, которого нет.
> - **Док. случай — P1P4 3657e8ab:** ранжировка ставила Bilbo, Luckwearer (60.9, топ-1;
>   у 74% победителей — § 4.8), совет ушёл в Mirkwood Nurturer 55.6 «по цветам» пула из
>   3 карт. Синий тёк весь драфт (13 сильных моносиних отданы с пика ≥5, включая второго
>   Bilbo на P3P6), Nurturer'ов после этого пришло ещё три — заменяемая карта против
>   незаменяемой. Игрок подтвердил: взял бы Бильбо. Там же P3P2: 4-я Nurturer над
>   Mirkwood Pathmaker 61.7 (−6.1) — знакомая карта вместо сильнейшей кастуемой.

**Служебный скан пака (механика, не рассуждение):** прочитать пак целиком, отметить топ в цвете, топ вне цвета, карты с `⚠trap` и метки `⟳в пуле ×N` — чтобы ничего не пропустить физически.
...
**Hard rule: max 2 copies of any common/uncommon in the 40.** At 2+ copies, that card doesn't exist in this pack — evaluate only the rest. Пак печатает `⟳в пуле ×N` у каждой уже взятой карты (внесено 20.08.2026: правило жило прозой и было нарушено — 4-я Mirkwood Nurturer над Pathmaker 61.7, драфт 3657e8ab); исключение из «2 копий» — только карта ядра пары с потолком копий у победителей выше (⚑ КОПИИ молчит) И названная причина-роль.

**Bad pack rule:** if nothing feeds the anchor and nothing fills a gap — take the card most likely to wheel or signal an open color for the next pack. Don't default to "highest GIH filler". (Раньше в этом списке стоял «high IWD» — снято 18.08.2026 вместе с тайбрейком.)

---

## ⚔️ ЧЕМ ЭТА КОЛОДА ВЫИГРЫВАЕТ — ОТВЕТ ОБЯЗАН БЫТЬ С НАЧАЛА 2-го БУСТЕРА (внесено 20.08.2026)

> **Указание пользователя:** «скилл при сборке и в драфте начиная с середины (2-3 пак) должен
> уже понимать, чем колода выигрывает, её реальный план, а не собирать мягкую середину».
> И отдельно: «предпочитать мощные тела мана-кривой; хороший кёрв нужен только атакующей
> колоде, все остальные должны жертвовать кривой».

**Замер, который это подтверждает (298 трофейных листов HOB):**
**93.3% победителей вытягивают ХОТЯ БЫ ОДИН маршрут победы до медианы своей пары.**
Листов «ни одного маршрута» — 20 из 298 (**6.7%**). Мягкая середина перестала быть
оценочным суждением: это измеримое состояние, и у победителей оно редкое.

**Четыре маршрута** (медианы — ПО ПАРЕ, из `<set>_traps.json` → `routes`; печатает `⚑ ПЛАН ПОБЕДЫ`):

| маршрут | чем меряется | пример нормы по HOB |
|---|---|---|
| **КРУПНЫЕ ТЕЛА** | тел силой ≥4 | WU 0 · UB 2 · **UR 4** · WR/BG/UG 4 · BR 6 · RG 7 |
| **ВОЗДУХ** | суммарная сила тел с ПЕЧАТНЫМ эвейжном (за ману не считается) | WU 8 · UR 11 · BR 17 · RG 21 |
| **ШИРИНА/АГРО** | существ И дешёвых тел одновременно на медиане | UR 15 существ / 4 дешёвых |
| **ОТВЕТЫ/ГРАЙНД** | removal (hard+soft) | UR 4 · WR/WB 5 |

**Правила пика, вытекающие отсюда:**

1. **С P2P1 маршрут называется ВСЛУХ в строке `СОБИРАЕМ`.** Не «собираем UR», а «выигрываем
   крупными телами / воздухом / шириной». Не можешь назвать — это и есть диагноз.
2. **🔴 ТЕЛО СИЛОЙ ≥4 БЬЁТ ФИЛЛЕР В КРИВУЮ ВЕЗДЕ, КРОМЕ ПОЛОСЫ C (агро).** Корреляция
   «тел силой ≥4» ↔ «существ cmc≤2» у победителей **−0.28** — они РАЗМЕНИВАЮТ эти оси,
   а не набирают обе. Хороший кёрв нужен колоде, которая атакует; всем остальным он
   покупается ценой способности убить, и это плохая сделка.
3. **`⚑ КРИВАЯ` при открытой полосе крупных тел САМА отключается** — если она молчит,
   не подставляй квоту обратно из головы.
4. **Углубляй ОДИН маршрут, не выравнивай четыре.** Ровный профиль по всем осям — это
   портрет тех самых 6.7%, а не «сбалансированная колода».
5. **`⚑ ЧЕМ УБИВАЮ` ниже минимума пары = красный.** Тела берутся ТОЛЬКО пиком; на сборке
   этого уже не добрать, и никакая метрика кривой не компенсирует.

> **Док. случай, из которого всё выведено — драфт eba1b036, счёт 0-3.** Пул давал **три
> открытых маршрута** (крупные тела 5 при норме 4 · воздух 20 при 11 · ширина 15 при 15).
> Собранная из него колода имела **НОЛЬ**: три тела силы ≥4 вырезаны на сборке со ссылками
> на кривую. В обеих доигранных партиях она нанесла **ровно ноль урона** (оппонент 20→22).
> Ни один прибор до 20.08.2026 этого не говорил — все восемь осей аудита показывали
> «в диапазоне». Разбор — `JOURNAL.md` § 8.17.
...
> | **GU** | Эльфы · **Landfall** | ~~доп. земельные дропы~~ → **пейофф-тела** (см. ниже) | B (инкремент) или A |
...
>
> ### 🔴 ПРАВИЛО НЕ ЗАКАНЧИВАЕТСЯ НА ПЕРВОМ БУСТЕРЕ (внесено 16.08.2026, разбор драфта 31a78cee)
> **Дыра, которую это чинит:** порог выше откалиброван только на P1 («до P1P10, пул ≤9»), и во
> втором-третьем бустере правило молчало. Задокументированный результат: инструмент **13 раз**
> сказал «сильнее вне цвета» (4 из них ⚑ПИВОТ), все 13 проигнорированы поодиночке, и колода
> осталась в WU, пока чёрный тёк весь драфт. Замер по тому же драфту: чёрный дал **6 карт GIH≥58
> на пиках 5+ при среднем 55.4** — лучший показатель из пяти цветов, и тёк во ВТОРОМ **и** в
> ТРЕТЬЕМ бустере, то есть был не додрафтен с обеих сторон стола.
>
> **Три реакции вместо одной. Отвечать ЛЕСТНИЦЕЙ — от дешёвой к дорогой:**
>
> | Когда | Что рассматривать | Цена |
> |---|---|---|
> | P1, пул ≤9 | **ПИВОТ** — полная смена цвета | 2–4 карты |
> | P2, пул 10–20 | **СПЛЕШ** по Rule of Three, если карта в **1 off-пип**; пивот только при разрыве ≥8 GIH | 1–2 слота + фикс |
> | P3 | **СПЛЕШ** одной бомбы, если фикс уже есть; иначе осознанный отказ вслух | 1 слот |
> | **любой бустер, вкл. P3** | **СМЕНА ВТОРОГО ЦВЕТА при сохранении базового — это НЕ пивот из этой таблицы** (внесено 22.08.2026, драфт 91a4b8e8): база и все гибриды сохраняются, фикс не нужен (второй цвет = базовые земли). Команду даёт `⚑ ОСЬ-КОНФЛИКТ` — он же считает цену и потери числом | только моно-карты старого второго цвета (обычно 3–6, в мейн из них шло меньше половины) |
>
> **⚠️ Эта строка добавлена потому, что лестница выше ЗАПРЕЩАЛА правильное решение:** в драфте
> 91a4b8e8 смена U→G на P3 стоила 4-6 заменимых синих карт (все ≤59.9 GIH), дала колоду 5W-3L —
> а по старой таблице на P3 разрешался только «сплеш бомбы», и агенты-переигровщики, следуя ей,
> «легально» отказывались (JOURNAL § 8.34-8.35). Полный пивот (смена БАЗЫ) в P3 по-прежнему
> закрыт — дёшев только второй цвет.
>
> **Ключевое, что было упущено:** «поздно пивотить» ≠ «нечего делать». В разобранном драфте
> **5 из 6 пропущенных бомб стоили ОДИН чёрный пип** (`Desolation Prowler` {1}{B} 61.1 ·
> `Down, Down to Goblin-town` {2}{B} 60.2 · `Ravening Warg` {1}{B} 58.1 · `Gollum the Abandoned`
> {1}{B} 58.3 · `Dáin's Company` {R}{W} 64.5) — они **сплешабельны**, и пивот для них не требовался.
> Флаг `~splash` в паке означает ровно это; § Оценка карты уже запрещает валить `~splash`
> и `✗offcolor` в одну корзину — но при живом драфте это правило не применялось ни разу.
>
> **Инструмент теперь помнит за тебя — баннер `⚑ ПАСУЕМ <цвет>`** (`draft_live.passed_color_banner`).
> Он копит через все пройденные паки и загорается на **3-й** отданной карте GIH≥58 одного цвета,
> отдельной строкой называя **сплешабельные в один пип**. Пер-пак баннеры этого не видели: каждое
> срабатывание по отдельности защитимо, тринадцать подряд — нет. На разобранном драфте загорелся
> бы на **P2P2** (3-я карта), к P2P6 показал бы 5, к P3P1 — 8.
> **Когда он горит, строка `ПИВОТ:` в блоке пика ОБЯЗАТЕЛЬНА** (см. § How to give a pick).
>
> ⚠️ Всё это выведено из **одного** драфта (n=1) — статус гипотезы по § КАЛИБРОВКА закон 4.
> Проверять: если следующие 2–3 драфта покажут, что сплеш по этому правилу ухудшает мана-базу
> (голдфиш: скрю/кастуемость), порог поднять или правило снять — и записать в `rules_graveyard.md`.
...
- **Когда коммитить 2 цвета.** P1P1–P5 сигналов об открытости ещё нет (паки свежие) → бери **СИЛУ / гибкость / colorless / фикс, НЕ лочь 2 цвета.** С пика 6 читай, что течёт и колесит → коммить в лейн, который **одновременно ОТКРЫТ и МОЩНЫЙ**, к ~пику 6–8. **Исключение — Полоса B (движок): коммить РАНО, P1–P2** (плотность нужна с первых пиков, иначе выйдет shallow = мягкая середина). Строгий CABS учит «decide colors early» — на Arena это верно **только для движка**; для A/C late-commit по сигналам сильнее (реагируешь на то, что реально раздают).
  - **🔴 КОММИТ ФИКСИРУЕТ БАЗОВЫЙ ЦВЕТ, А НЕ ПАРУ (внесено 22.08.2026, драфт 91a4b8e8; = память [[mtg-anchor-one-color-stay-open]], которая в этом файле раньше не стояла).** Ярлык пары в `СОБИРАЕМ` — рабочая гипотеза, а не обязательство: **второй цвет остаётся пересматриваемым параметром до конца драфта**, его смена дешева (база+гибриды сохраняются, фикс не нужен) и командуется баннером `⚑ ОСЬ-КОНФЛИКТ`. Инерция ярлыка задокументирована дважды: живой советчик держал «UB» весь драфт при оси Волк 5-7, и агенты-переигровщики отказывались от смены со ссылкой «уже закоммичен» — оба раза слово «коммит» читалось как про пару, хотя обязательство создаёт только база.
...
- **🪦 «Floor vs ceiling: при близком GIH бери высокий IWD» — СНЯТО 18.08.2026** (`rules_graveyard.md` § 9). Проверено на 294k паков: на GIH-ничьих высокий IWD берут 52.2% трофейщиков и 55.4% провальщиков — признак скорее провала, чем трофея. **Что осталось от IWD:** флаг `⚠trap` при IWD<0 («карта вредит, пока нет поддержки») и маркер ЯКОРЯ полосы B (IWD ≥+6 при ALSA ≤3.6) — это другие оси, не тайбрейк. Ничью решают роль, дыра, план полосы и квадранты.
- **Castability beats raw GIH — но НЕ смешивай single-pip и double-pip off-color (баг, пойманный 13.07.2026).** `~splash` (ОДИН off-color пип, напр. {3}{R}) — реально живая опция при 3+ источниках (Rule of Three), а **на пике ≤~15 из ~33 времени добрать фикс ещё достаточно** — бомба здесь заслуживает статуса реальной альтернативы, а не «нереалистично», особенно если полоса ещё не жёстко законтрена. `✗offcolor` (ДВА+ off-color пипа, напр. {B}{B}) — вот это реально **не сплешится никогда**, дабл-пип сжирает мана-базу. Не приравнивай эти два случая — ошибка была списать одинарный-пип бомбу как «нереалистичный сплеш» той же фразой, что и дабл-пип. Исключение: `✗offcolor` на пике ≤5 при 0 коммите = сигнал ПИВОТИТЬ, а не брать.
...
- **Juza tiebreaker:** при равных якорь/дыра/квадранты — бери ДЕШЕВЛЕ (разыгрывается в большем числе партий).
...
- **Justified risk — когда оправдан риск:**
  - **Слабый пул (пики 1–5 все B-tier, нет якоря):** build-around с высоким потолком > надёжный B-tier (обе медиокрные колоды проигрывают — но рискованная хоть иногда выигрывает крупно). ⚠️ «Высокий потолок» здесь — это ЯКОРЬ полосы по маркеру IWD≥+6/ALSA≤3.6, а не «просто IWD повыше»: тайбрейк по IWD похоронен.
