...
def main():
    ...
    if misses:
        print("⚠ КАРТЫ ИЗ --pool НЕ НАЙДЕНЫ В СВОИХ ПАКАХ — за эти пики выбрал бот, "
              "пул дальше НЕ ТВОЙ:")
        for pp, pk_, nm in misses[:6]:
            print(f"    P{pp}P{pk_}: {nm}")
        print("    Проверь написание и порядок: карты в --pool должны идти в том порядке, "
              "в каком ты их брал.\n")
    if pk is None:
