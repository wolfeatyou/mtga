...
> **Учёт партий (с 18.08.2026, JOURNAL § 8.13) — автоматический:** `analyze_game.py` и
> `match_watch.py` при старте сами архивируют всё новое из лога в реестр
> `matches/<set>_ledger.jsonl` (результат, ходы, событие, сабмиченная колода поимённо)
> и сырые сегменты `logs/<match8>.log.gz` — разбор возможен и после ротации.
> После игровой сессии БЕЗ разбора прогнать руками: `python3 match_archive.py <set>`;
> счёт по событиям и колодам: `--status`. Категоризацию поражений в `<set>_match_log.md`
> это НЕ заменяет — реестр это факты, матч-лог это выводы.
...
> **⚡ ПЕРЕД МАТЧЕМ / при живом коучинге — читать `<set>_pilot.md`** (`msh_pilot.md`).
> И прогнать **`python3 race_card.py <лист> --set <set>`** (§ 8.25): наши часы против
> медианных часов пар меты — вердикт «гонщик/защитник» по каждому вероятному матчапу
> числом, а не ощущением. Вердикт против эквип/amass-пар читать осторожнее (§ 8.23). Минимум — блок «четыре вопроса каждый ход» + раздел под матчап. Это не то же, что `insights`: там «во что собирать», здесь «как разыгрывать». Обоснование приоритета: прогон 19.07 показал сборку уровня 8 при счёте 3W–3L, где 2 из 3 поражений имели окно — **узкое место в розыгрыше, не в билде.**
