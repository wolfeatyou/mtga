...
def combo_banner(ids, by_id, ratings, picks, max_lines=3, main=None, pnum=None, pick=None):
    ...
    if gaps and hits:
        ...
        out.append(f"⚠ РОЛЬ ВПЕРЁД: {g}. "
                   f"Связка ниже НЕ повод пропускать роль: в слепом A/B 17.08 судья трижды "
                   f"назвал причиной проигрыша именно роль — «летунов 3 против 7, упирается "
                   f"в стену», «удаления почти нет», «синергия мёртвая: Эльфов в колоде два».")
